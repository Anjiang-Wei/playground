...
task toplevel()
  ...
  var p_links = preimage(r_links, p_page_full, r_links.dest_node)
  ...
  while not converged do
    ...
    if num_iterations > config.max_iterations then
      ...
      for color in colors do
        compute_next_rank(p_page_dest[color], p_links[color], config.damp, config.error_bound, config.num_pages)
      end
      for color in colors do
